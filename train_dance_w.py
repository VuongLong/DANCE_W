...
from models.algorithms import KanNet, op_copy, lr_scheduler
from wasserstein_loss import entropic_wasserstein_loss
...
kan_net = KanNet(output=1, bottleneck_dim=2048)
# for kan net parameters
...
for key, value in dict(G.named_parameters()).items():
    if value.requires_grad and "features" in key:
        ...
        if 'bias' in key:
            kan_param_group += [{'params': [value], 'lr': conf.train.multi,
                        'weight_decay': conf.train.weight_decay}]
        else:
            kan_param_group += [{'params': [value], 'lr': conf.train.multi,
                        'weight_decay': conf.train.weight_decay}]
        ...
        if 'bias' in key:
            kan_param_group += [{'params': [value], 'lr': 1.0,
                        'weight_decay': conf.train.weight_decay}]
        else:
            kan_param_group += [{'params': [value], 'lr': 1.0,
                        'weight_decay': conf.train.weight_decay}]
...
for k, v in kan_net.named_parameters():
    if lr_decay1 > 0:
        kan_param_group += [{'params': v, 'lr': 1e-2 * lr_decay2}]
    else:
        v.requires_grad = False

kan_optimizer = optim.SGD(kan_param_group)
kan_optimizer = op_copy(kan_optimizer)
...
def train():
    ...
    for step in range(conf.train.min_step + 1):
        ...
        kan_val = kan_net(feat_t)
        lost_ot = entropic_wasserstein_loss(feat_t, feat, kan_val, 36)
        total_loss = loss_s# + lost_ot
        ...
        kan_optimizer.step()
        kan_optimizer.zero_grad()
        lr_scheduler(kan_optimizer, iter_num=step, max_iter=conf.train.min_step + 1)
